	...
	let 	lists 				= {},
			...
			deleteBtns 			= document.getElementsByClassName('btn-delete'),
	...
	function setActiveList(activeListLink ){
		if (activeListLink) {
			...
			lastListId = +/\d+/.exec(keys[keys.length-1]);
			activeThisList(lastListId);
		} 
	}
	...
	function deleteThisList(id){
		let bdCard = document.getElementById('bd-card-id-'+id);
		//bdCard.parentNode.removeChild(bdCard);
		console.log(bdCard);
	...
	function createListener(){
		...
		if(a===0){
			...
			for (index = 0; index < deleteBtns.length; index++) {
			    button = deleteBtns[index];
			    button.addEventListener('click', function (event) {
			        event.preventDefault();
			        console.log('deletebuton activate')
				    deleteThisList(this.dataset.listid);
			    });
			}
	...
	function createNewList(title){
	    let output = ""; 
	    output += '<div class="row justify-content-around bd-card" id="bd-card-id-'+listId+'">'
	    output += '<div class="col-12 title-list"><h3>'+ title + '</h3><div><button data-listid='+ listId +' type="button" class="btn btn-warning btn-edit"><i class="fas fa-pen-fancy"></i></button><button data-listid='+ listId +' type="button" class="btn btn-danger btn-delete"><i class="fas fa-trash"></i></button></div></div>'
